...
export const getAllBills = async (userId: string, isDes = false) => {
  ...
  */
  const res: (z.infer<typeof BillSchema> & {
    userName: string;
    userEmail: string;
    id: string;
  })[] = (await db
    ...
    .where(
      eq(isDes ? bills.designerId : bills.clientId, userId)
    )) as unknown as (z.infer<typeof BillSchema> & {
    userName: string;
    userEmail: string;
    id: string;
  })[];
...
export const updateBill = async (invoice: any, changed: any) => {
  ...
  if (Object.keys(update).length > 0) {
    ...
    console.log(update);
    await db.update(bills).set(update).where(eq(invoice.id, bills.id));
    return 204;
  } else {
    // NO UPDATES
    return 304;
